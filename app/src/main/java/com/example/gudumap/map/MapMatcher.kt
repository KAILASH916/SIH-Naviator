...
class MapMatcher(context: Context? = null) {
    ...
    fun loadRoadsFromAssets(context: Context) {
        try {
            ...
            for (i in 0 until roadsArray.length()) {
                ...
                val rawName = roadObj.optString("name", "")
                val name = if (rawName.isBlank() || rawName.equals("Coimbatore Road", ignoreCase = true)) {
                    "Unnamed road"
                } else {
                    rawName
                }
            ...
            Log.i(TAG, "Loaded ${roads.size} Coimbatore road segments for map matching (metadata count: ${json.optInt("roads_count", roads.size)}).")
        } catch (e: Exception) {
    ...
    fun match(
        ...
        // The bundled road graph only covers Coimbatore. Outside that area, preserve the
        // actual GPS/dead-reckoning position instead of snapping it to an unrelated road.
        if (estimatedLat !in OfflineMapManager.COIMBATORE_BOUNDS.latSouth..OfflineMapManager.COIMBATORE_BOUNDS.latNorth ||
            estimatedLon !in OfflineMapManager.COIMBATORE_BOUNDS.lonWest..OfflineMapManager.COIMBATORE_BOUNDS.lonEast
        ) {
        ...
        var bestRoadId: Long? = null

        val isPedestrianOrSlow = speedKmh < 3f
        val maxDistThreshold = if (isPedestrianOrSlow) 25.0 else MAX_DISTANCE_THRESHOLD_METERS
        ...
        for (road in roads) {
            ...
            for (i in 0 until points.size - 1) {
                ...
                // Segment spatial bounding box filter (with 0.002deg / ~220m buffer)
                // Correct segment crossing check: skip only if point is strictly outside segment's bounding box
                val minLat = minOf(p1.lat, p2.lat) - 0.002
                val maxLat = maxOf(p1.lat, p2.lat) + 0.002
                val minLon = minOf(p1.lon, p2.lon) - 0.002
                val maxLon = maxOf(p1.lon, p2.lon) + 0.002

                if (estimatedLat < minLat || estimatedLat > maxLat ||
                    estimatedLon < minLon || estimatedLon > maxLon
                ) {
                    continue
                }
                ...
                if (dist < maxDistThreshold && dist < bestDistance) {
                    ...
                    if (minHeadingDiff <= MAX_HEADING_DIFF_DEG || isPedestrianOrSlow) {
                        val distScore = (1.0 - (dist / maxDistThreshold)).coerceIn(0.0, 1.0).toFloat()
                        val angleScore = (1.0 - (minHeadingDiff / MAX_HEADING_DIFF_DEG)).coerceIn(0.0, 1.0).toFloat()
                        
                        // Scoring weights: walking/slow speed relies mostly on proximity, vehicle mode balances heading
                        var confidence = if (isPedestrianOrSlow) {
                            distScore * 0.85f + angleScore * 0.15f
                        } else {
                            distScore * 0.6f + angleScore * 0.4f
                        }

                        // Continuity hysteresis: boost confidence if staying on the previously matched road
                        if (lastMatchedRoadId != null && road.id == lastMatchedRoadId) {
                            confidence += 0.15f
                        }
        ...
        val shouldSnap = bestConfidence >= MIN_CONFIDENCE_SNAP && bestDistance < maxDistThreshold

        return if (shouldSnap) {
            lastMatchedRoadId = bestRoadId
            MapMatchResult(
                matchedLat = bestSnappedLat,
                matchedLon = bestSnappedLon,
                roadName = if (bestRoadName == "Unnamed road") "" else bestRoadName,
            ...
            MapMatchResult(
                matchedLat = estimatedLat,
                matchedLon = estimatedLon,
                roadName = if (bestDistance < 80.0 && bestRoadName != "Unnamed road") bestRoadName else "",
                confidence = 0f,
    ...
    private fun distanceBetweenMeters(lat1: Double, lon1: Double, lat2: Double, lon2: Double): Double {
        ...
        val dLon = Math.toRadians(lon1 - lon2) // Correct formula for haversine lon
