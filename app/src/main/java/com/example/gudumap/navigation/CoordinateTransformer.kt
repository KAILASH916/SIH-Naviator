...
class CoordinateTransformer {
    ...
    companion object {
        ...
        /**
         * Computes the initial bearing (forward azimuth) from (lat1, lon1) to (lat2, lon2) in degrees [0, 360).
         */
        fun computeInitialBearing(lat1: Double, lon1: Double, lat2: Double, lon2: Double): Float {
            ...
            return normalizeHeading(bearingDeg)
        }
        ...
        /**
         * Normalizes a geographic heading in degrees [0..360) clockwise from North.
         *   0°   = North (up)
         *   90°  = East (right)
         *   180° = South (down)
         *   270° = West (left)
         */
        fun normalizeHeading(headingDeg: Float): Float {
            val normalized = (headingDeg % 360f + 360f) % 360f
            return if (normalized == 360f) 0f else normalized
        }

        /**
         * Converts geographic heading (clockwise from North: 0°=North/UP, 90°=East/RIGHT, 180°=South/DOWN, 270°=West/LEFT)
         * into osmdroid marker canvas rotation angle.
         *
         * The vector drawables (ic_navigation_arrow and ic_navigation_arrow_red) naturally point straight UP (North, 0°).
         * osmdroid rotates flat markers clockwise on canvas by mRotation degrees.
         * Therefore, geographic heading directly maps to marker canvas rotation.
         */
        fun computeMarkerRotation(headingDeg: Float, drawableBaselineOffsetDeg: Float = 0f): Float {
            val normHeading = (headingDeg % 360f + 360f) % 360f
            val osmdroidCanvasAngle = (360f - normHeading) % 360f
            return normalizeHeading(osmdroidCanvasAngle + drawableBaselineOffsetDeg)
        }
         ...
         */
        fun computeShortestAngularDelta(currentDeg: Float, targetDeg: Float): Float {
            return ((targetDeg - currentDeg + 540f) % 360f) - 180f
        }
    }
    ...
    /**
     * Instance delegation for addMetricDisplacementToGeodetic.
     */
    fun addMetricDisplacementToGeodetic(
        latDeg: Double,
        lonDeg: Double,
        ...
        return Companion.addMetricDisplacementToGeodetic(latDeg, lonDeg, deltaNorth, deltaEast)
    }
}
