...
class ZuptDetector(
    ...
    var estimatedGyroBiasX: Float = 0f
        private set
    var estimatedGyroBiasY: Float = 0f
        private set
    var estimatedGyroBiasZ: Float = 0f
        private set

    private var gyroBiasSampleCount = 0
    private var gyroBiasAccumX = 0f
    private var gyroBiasAccumY = 0f
    private var gyroBiasAccumZ = 0f
    ...
    fun update(sample: ImuSample, gnssSpeed: Float? = null): Boolean {
        ...
        // Handles both raw acceleration (around 9.81 m/s^2) and gravity-removed linear acceleration (around 0 m/s^2)
        val accMagDev = kotlin.math.min(accMag, kotlin.math.abs(accMag - 9.81f))
        val accCondition = (accMagDev < accMagnitudeThreshold) && (accVar < accVarianceThreshold)
        ...
        if (instantStationary) {
            ...
            gyroBiasAccumY += sample.gy
            gyroBiasAccumZ += sample.gz
            gyroBiasSampleCount++
            if (gyroBiasSampleCount >= 5) {
                estimatedGyroBiasX = gyroBiasAccumX / gyroBiasSampleCount
                estimatedGyroBiasY = gyroBiasAccumY / gyroBiasSampleCount
                estimatedGyroBiasZ = gyroBiasAccumZ / gyroBiasSampleCount
            }
    ...
    fun reset() {
        ...
        gyroBiasAccumY = 0f
        gyroBiasAccumZ = 0f
        estimatedGyroBiasX = 0f
        estimatedGyroBiasY = 0f
        estimatedGyroBiasZ = 0f
    }
}
