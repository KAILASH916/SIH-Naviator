...
class SensorManager(
    ...
    private val uncalibratedGyroscope =
        sensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE_UNCALIBRATED)

    private val rawGyroscope =
    ...
    fun startGyroscope(callback: (GyroscopeData) -> Unit) {
        ...
        val sensorToRegister = uncalibratedGyroscope ?: rawGyroscope
        sensorToRegister?.let { sensor ->
            sensorManager.registerListener(
                this,
                sensor,
    ...
    fun stopGyroscope() {
        uncalibratedGyroscope?.let { sensorManager.unregisterListener(this, it) }
        rawGyroscope?.let { sensorManager.unregisterListener(this, it) }
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        ...
        when (event.sensor.type) {
            ...
            Sensor.TYPE_GYROSCOPE_UNCALIBRATED -> {
                // event.values[0..2] is uncalibrated angular rate (rad/s), event.values[3..5] is estimated hardware drift/bias
                val gx = if (event.values.size >= 6) event.values[0] - event.values[3] else event.values[0]
                val gy = if (event.values.size >= 6) event.values[1] - event.values[4] else event.values[1]
                val gz = if (event.values.size >= 6) event.values[2] - event.values[5] else event.values[2]
            ...
            Sensor.TYPE_GYROSCOPE -> {
                // If uncalibrated hardware gyroscope is active, ignore standard raw gyroscope
                if (uncalibratedGyroscope != null) return
